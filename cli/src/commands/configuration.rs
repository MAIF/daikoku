...
use crate::{
    ...
    utils::{absolute_path, get_home, get_option_home},
    ConfigCommands,
};

pub(crate) const DAIKOKU_SERVER: &str = "DAIKOKU_SERVER";
pub(crate) const DAIKOKU_CLIENT_ID: &str = "DAIKOKU_CLIENT_ID";
pub(crate) const DAIKOKU_CLIENT_SECRET: &str = "DAIKOKU_CLIENT_SECRET";
pub(crate) const DAIKOKU_PATH: &str = "DAIKOKU_PATH";
...
pub(crate) fn get_sources_root() -> Option<String> {
    if let Ok(values) = read_configuration() {
        Some(values.get("DAIKOKU_PATH").map(|p| p.clone()).unwrap_or("./cms".to_string()))
    } else {
        None
    }
}
