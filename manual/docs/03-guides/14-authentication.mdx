...
<Tabs>
  ...
  :::
    ...
    Scroll to the Application URLs section and add the following url as Allowed Callback URLs : http://localhost:9000/auth/oauth2/callback
    Set http://localhost:9000 as Allowed Logout URLs
    Set http://localhost:9000 as Allowed Web Origins
    ...
    - add app metadata to to a user `daikokuAdmin` with `true` as value
