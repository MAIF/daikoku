...
Now that you know [how to configure Daikoku with the config. file](./configfile.md) every property in the following block can be overridden by an environment variable (an env. variable is written like `${?ENV_VARIABLE}`).
