...
Daikoku needs a database to run. Before run it, think to start your Postgres instance and setup a database with the right name (cf. [configuration](../05-firstrun/configfile.md#db-configuration))
...
<Tabs>
  ...
  <TabItem value="docker" label="From docker">
    ...
    You can also provide some ENV variables using the `--env` flag to customize your Daikoku instance.
