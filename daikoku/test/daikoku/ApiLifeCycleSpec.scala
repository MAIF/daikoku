  ...
  "API life cycle" must {
    ...
    "setup isDefault for another version if blocked API is currently isDefault" in {
      ...
      (visibleApisAfter.json \ "data" \ "visibleApis" \ "total")
        .as[Int] mustBe 1
      val apiAfter =
        (visibleApisAfter.json \ "data" \ "visibleApis").as[JsArray].value.head
      (apiAfter \ "name").as[String] mustBe "api-test-isDefault"
      (apiAfter \ "currentVersion").as[String] mustBe "2.0.0"
      (apiAfter \ "state").as[String] mustBe "published"
      (apiAfter \ "isDefault").as[Boolean] mustBe true
