...
class DaikokuSuites extends Suite with BeforeAndAfterAll { thisSuite =>
  ...
  val defaultTestConfig = Configuration(ConfigFactory
    .parseString(s"""
       |{
       |  daikoku.tenants.provider = "hostname"
       |  mongodb {
       |    # uri = "mongodb://localhost:27017/daikoku-tests-${System.currentTimeMillis()}"
...
//      new BasicUsageSpec(defaultTestConfig),
  ...
  override val nestedSuites: collection.immutable.IndexedSeq[Suite] = Vector.empty ++ getSuites()
...
object utils {
  ...
  class DaikokuTestComponentsInstances(context: ApplicationLoader.Context, conf: Configuration => Configuration)
  ...
  trait DaikokuSpecHelper { suite: OneServerPerSuiteWithMyComponents =>
    ...
    def await(duration: FiniteDuration): Unit = {
      val p = Promise[Unit]
      daikokuComponents.env.defaultActorSystem.scheduler.scheduleOnce(duration) {
        p.trySuccess(())
      }
    ...
    def flush(): Future[Unit] = {
      for {
        ...
        _ <- daikokuComponents.env.dataStore.apiSubscriptionRepo.forAllTenant().deleteAll()
        _ <- daikokuComponents.env.dataStore.apiDocumentationPageRepo.forAllTenant().deleteAll()
        _ <- daikokuComponents.env.dataStore.notificationRepo.forAllTenant().deleteAll()
        _ <- daikokuComponents.env.dataStore.consumptionRepo.forAllTenant().deleteAll()
        _ <- daikokuComponents.env.dataStore.auditTrailRepo.forAllTenant().deleteAll()
...
//      await(0.1.seconds)
    ...
    def setupEnv(
      ...
      for {
        _ <- flush()
        _ <- daikokuComponents.env.dataStore.userSessionRepo.deleteAll()
        _ <- Source(tenants.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.tenantRepo.save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(users.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.userRepo.save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(teams.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.teamRepo.forAllTenant().save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(apis.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.apiRepo.forAllTenant().save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(subscriptions.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.apiSubscriptionRepo.forAllTenant().save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(notifications.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.notificationRepo.forAllTenant().save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(consumptions.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.consumptionRepo.forAllTenant().save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(sessions.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.userSessionRepo.save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(resets.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.passwordResetRepo.save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
        _ <- Source(creations.toList).mapAsync(1)(i => daikokuComponents.env.dataStore.accountCreationRepo.save(i)(daikokuComponents.env.defaultExecutionContext)).toMat(Sink.ignore)(Keep.right).run()
      } yield ()
    }

    def logoutBlocking(user: User, on: Tenant): Unit = logoutBlocking(user.email, on)
    ...
    def logout(email: String, on: Tenant): Future[Unit] = {
      daikokuComponents.env.dataStore.userSessionRepo.delete(Json.obj(
        "userEmail" -> email
      )).map(_ => ())
    }
    ...
    def loginWithBlocking(email: String, on: Tenant): UserSession = loginWith(email, on).futureValue

    def loginWith(email: String, on: Tenant): Future[UserSession] = {
      daikokuComponents.env.dataStore.userRepo.findOneNotDeleted(Json.obj(
        "email" -> email
      )).flatMap {
        case None => FastFuture.failed(new RuntimeException("User not found !!!"))
        case Some(user) =>
          daikokuComponents.env.dataStore.teamRepo.forTenant(on).findOne(Json.obj()).map {
            case None => Team(
            ...
            case Some(team) => team
          }.flatMap(t => daikokuComponents.env.dataStore.teamRepo.forTenant(on).save(t).map(_ => t)).flatMap { team =>
            ...
            daikokuComponents.env.dataStore.userSessionRepo.save(session).map { _ =>
              session
            }
          }
      }
    ...
    def httpJsonCallBlocking(path: String, method: String = "GET", headers: Map[String, String] = Map.empty, body: Option[JsValue] = None)(implicit tenant: Tenant, session: UserSession): WSResponse = httpJsonCall(
      path,
      method,
      headers,
      body
    ...
    def httpJsonCall(_path: String, method: String = "GET", headers: Map[String, String] = Map.empty, body: Option[JsValue] = None)(implicit tenant: Tenant, session: UserSession): Future[WSResponse] = {
      val path = _path match {
        case str if str.contains("?") => str + "&sessionId=" + session.sessionId.value
      ...
      val builder = daikokuComponents.env.wsClient
        ...
        .withMethod(method)
      body.map(b => builder.withBody(b)).getOrElse(builder).withCookies(DefaultWSCookie(
        ...
        value = sign(
          ...
          Json.obj(
            ...
            "exp" -> ((System.currentTimeMillis() - 10000) + (5 * 60 * 1000)) / 1000,
    ...
    def httpJsonCallWithoutSessionBlocking(path: String, method: String = "GET", headers: Map[String, String] = Map.empty, body: Option[JsValue] = None)(implicit tenant: Tenant): WSResponse = httpJsonCallWithoutSession(
      path,
      method,
      headers,
      body
    ...
    def httpJsonCallWithoutSession(path: String, method: String = "GET", headers: Map[String, String] = Map.empty, body: Option[JsValue] = None)(implicit tenant: Tenant): Future[WSResponse] = {
      val builder = daikokuComponents.env.wsClient
        .url(s"http://127.0.0.1:$port$path")
        .withHttpHeaders((headers ++ Map("Host" -> tenant.domain)).toSeq: _*)
        .withFollowRedirects(false)
...
//      await(1.second)
      ...
      flush().onComplete {
        case Success(_) => r
        case Failure(exception) => Logger.error("Error during flushing db", exception)
      }
...
//      r
    ...
    def openPageBlocking(path: String)(implicit tenant: Tenant, session: UserSession): Document = openPage(path)(tenant, session).futureValue

    def openPage(path: String)(implicit tenant: Tenant, session: UserSession): Future[Document] = actionOnPage(path, Seq("--dump-dom"))(tenant, session).map(str => Scoup.parseHTML(str))

    def screenshotPageBlocking(path: String)(implicit tenant: Tenant, session: UserSession): String = screenshotPage(path)(tenant, session).futureValue

    def screenshotPage(path: String)(implicit tenant: Tenant, session: UserSession): Future[String] = actionOnPage(path, Seq("--screenshot", "--window-size=1920,1080"))(tenant, session).map { res =>
      Try(Files.createDirectory(new File("./target/screenshots").toPath))
      Files.move(
        new File("./screenshot.png").toPath,
        new File(s"./target/screenshots/screenshot-${DateTime.now().toString("yyyy-MM-dd-HH-mm-ss-SSS")}.png").toPath,
        StandardCopyOption.REPLACE_EXISTING
      )
      res
    }

    def actionOnPage(path: String, action: Seq[String])(implicit tenant: Tenant, session: UserSession): Future[String] = {
      val url = s"http://${tenant.domain}:$port$path?sessionId=${session.sessionId.value}"
      val execs = Seq(
      ...
      execs.find { exec =>
        ...
        try {
          val p = sys.process.Process.apply(exec, Seq("--headless", "--disable-gpu") ++ action ++ Seq(url)).run(log)
          if (p.exitValue() == 0) {
        ...
        case Some(_) => ()
        case None => promise.tryFailure(new RuntimeException("Failed to launch Chrome"))
      }
      promise.future
    }


    def otoroshiPathApiKeyQuotas(groupId: String, clientId: String) = s"/api/groups/$groupId/apikeys/$clientId/quotas"
    ...
    def otoroshiDeleteApikeyPath(groupId: String, clientId: String) = s"/api/groups/$groupId/apikeys/$clientId"
    ...
    def generateApi(version: String = "0", tenant: TenantId, teamId: TeamId, docIds: Seq[ApiDocumentationPageId]) = Api(
    ...
    val defaultApi: Api = generateApi("default", tenant.id, teamOwnerId, Seq.empty)
  }
}