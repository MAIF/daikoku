...
import fr.maif.otoroshi.daikoku.domain.{Team, TeamId, TeamType, UserWithPermission}
...
class TenantController(DaikokuAction: DaikokuAction,
  ...
  def namesOfTenants() = DaikokuAction.async(parse.json) { ctx =>
    ...
    val tenantIds: Seq[String] = ctx.request.body.as[JsArray].value.map(_.as[String])
    PublicUserAccess(AuditTrailEvent(s"@{user.name} has accessed tenant names for ${tenantIds.mkString(", ")}"))(ctx) {
  ...
  def allTenants() = DaikokuAction.async { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has accessed list of all tenants"))(ctx) {
  ...
  def tenantList() = DaikokuAction.async { ctx =>
    PublicUserAccess(AuditTrailEvent("@{user.name} has accessed simplified tenant list"))(ctx) {
      env.dataStore.tenantRepo.findAllNotDeleted().map { tenants =>
        Ok(JsArray(tenants.map { tenant =>
          val status: String = if (ctx.user.tenants.contains(tenant.id)) "ALREADY_JOINED" else "CAN_JOIN"
          Json.obj(
            "_id"    -> tenant.id.value,
            "name"   -> tenant.name,
            "desc"   -> tenant.style.map(_.description).getOrElse("").asInstanceOf[String],
            "title"  -> tenant.style.map(_.title).getOrElse(tenant.name).asInstanceOf[String],
            "status" -> status,
  ...
  def redirectToTenant(id: String) = DaikokuAction.async { ctx =>
    PublicUserAccess(AuditTrailEvent(s"@{user.name} has accessed tenant redirection to @{dest.name} - ${id}"))(ctx) {
      ...
      env.dataStore.tenantRepo.findByIdNotDeleted(id).flatMap {
                ...
                env.dataStore.teamRepo
                  ...
                  .save(
                    Team(
                      ...
                      users = Set(UserWithPermission(ctx.user.id, Administrator)),
        ...
        case None =>
          Errors.craftResponseResult("Tenant not found", Results.NotFound, ctx.request, env = env, tenant = ctx.tenant)
      }
    }
  }

  def oneTenant(tenantId: String) = DaikokuAction.async { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has accessed one tenant @{tenant.name} - @{tenant.id}"))(ctx) {
  ...
  def createTenant() = DaikokuAction.async(parse.json) { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has created a tenant @{tenant.name} - @{tenant.id}"))(ctx) {
  ...
  def deleteTenant(id: String) = DaikokuAction.async { ctx =>
    DaikokuAdminOnly(AuditTrailEvent("@{user.name} has logically deleted tenant @{tenant.name} - @{tenant.id}"))(ctx) {
      env.dataStore.tenantRepo.findByIdNotDeleted(id).flatMap {
        case Some(tenant) => {
          ctx.setCtxValue("tenant.name", tenant.name)
          ctx.setCtxValue("tenant.id", tenant.id)
          for {
            ...
            _ <- env.dataStore.apiSubscriptionRepo.forTenant(tenant).deleteAllLogically()
            _ <- env.dataStore.apiDocumentationPageRepo.forTenant(tenant).deleteAllLogically()
            _ <- env.dataStore.notificationRepo.forTenant(tenant).deleteAllLogically()
  ...
  def saveTenant(tenantId: String) = DaikokuAction.async(parse.json) { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has updated tenant @{tenant.name} - @{tenant.id}"))(ctx) {
  ...
  def currentTenant(teamId: String) = DaikokuAction.async { ctx =>
    TeamAdminOnly(AuditTrailEvent(s"@{user.name} has accessed the current tenant"))(teamId, ctx) { team =>
  ...
  def fetchOpenIdConfiguration() = DaikokuAction.async(parse.json) { ctx =>
    ...
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has fetch OIDC config from ${_url}"))(ctx) {
      ...
      (ctx.request.body \ "url").asOpt[String] match {
        ...
        case Some(url) => {
          env.wsClient.url(url).withRequestTimeout(10.seconds).get().map { resp =>
            if (resp.status == 200) {
              Try {
