...
class OtoroshiSettingsController(DaikokuAction: DaikokuAction,
  ...
  def otoroshisSettings() = DaikokuAction.async { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has accessed otoroshi settings list"))(ctx) {
      FastFuture.successful(Ok(JsArray(ctx.tenant.otoroshiSettings.map(_.asJson).toSeq)))
    }
  }

  def otoroshiSettings(otoroshiId: String) = DaikokuAction.async { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has accessed one otoroshi settings ($otoroshiId)"))(ctx) {
  ...
  def deleteOtoroshiSettings(otoroshiId: String) = DaikokuAction.async { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has deleted one otoroshi settings ($otoroshiId)"))(ctx) {
      ctx.tenant.otoroshiSettings.find(_.id.value == otoroshiId) match {
        case Some(otoroshiSettings) =>
          env.dataStore.tenantRepo
            .save(
              ctx.tenant.copy(otoroshiSettings = ctx.tenant.otoroshiSettings.filterNot(_.id == otoroshiSettings.id))
            )
            .map { _ =>
              Ok(Json.obj("done" -> true))
            }
        case None => FastFuture.successful(NotFound(Json.obj("error" -> "Otoroshi not found")))
      }
    }
  }

  def saveOtoroshiSettings(otoroshiId: String) = DaikokuAction.async(parse.json) { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has updated one otoroshi settings ($otoroshiId)"))(ctx) {
      ctx.tenant.otoroshiSettings.find(_.id.value == otoroshiId) match {
        case None => FastFuture.successful(NotFound(Json.obj("error" -> "OtoroshiSettings not found")))
        case Some(_) =>
          OtoroshiSettingsFormat.reads(ctx.request.body) match {
            case JsError(e) => FastFuture.successful(BadRequest(Json.obj("error" -> "Error while parsing payload")))
            case JsSuccess(settings, _) => {
              env.dataStore.tenantRepo
                .save(
                  ctx.tenant
                    .copy(otoroshiSettings = ctx.tenant.otoroshiSettings.filterNot(_.id == settings.id) + settings)
                )
                .map { _ =>
                  Ok(settings.asJson)
                }
  ...
  def createOtoroshiSettings() = DaikokuAction.async(parse.json) { ctx =>
    DaikokuAdminOnly(AuditTrailEvent(s"@{user.name} has created one otoroshi settings (@{otoroshi.id})"))(ctx) {
      OtoroshiSettingsFormat.reads(ctx.request.body) match {
        case JsError(e) => FastFuture.successful(BadRequest(Json.obj("error" -> "Error while parsing payload")))
        case JsSuccess(settings, _) => {
          ctx.setCtxValue("otoroshi.id", settings.id)
          env.dataStore.tenantRepo
            .save(ctx.tenant.copy(otoroshiSettings = ctx.tenant.otoroshiSettings + settings))
            .map { _ =>
              Ok(settings.asJson)
            }
        }
  ...
  def otoroshiGroupsFor(teamId: String, oto: String) = DaikokuAction.async { ctx =>
    TeamAdminOnly(AuditTrailEvent(s"@{user.name} has accessed groups of one otoroshi settings ($oto) for team @{team.name} - @{team.id}"))(teamId, ctx) { team =>
  ...
  def createTestingApiKey(teamId: String) = DaikokuAction.async(parse.json) { ctx =>
    TeamAdminOnly(AuditTrailEvent(s"@{user.name} create a testing apikey for team @{team.name} - @{team.id}"))(teamId, ctx) { team =>
      ...
      (otoroshiSettingsOpt, serviceGroupOpt, clientNameOpt, tagOpt, apiOpt) match {
        case (Some(otoroshiSettings), Some(serviceGroup), Some(clientName), Some(tag), Some(apiId)) => {
          ctx.tenant.otoroshiSettings.find(s => s.id.value == otoroshiSettings) match {
            case None => FastFuture.successful(NotFound(Json.obj("error" -> s"Settings $otoroshiSettings not found")))
            case Some(settings) => {
              otoroshiClient.getServiceGroup(serviceGroup)(settings).map { group =>
                Some(group)
              } recover {
                case _ => None
              } flatMap {
                case None => FastFuture.successful(NotFound(Json.obj("error" -> "Group not found")))
                case Some(group) => env.dataStore.apiRepo.forTenant(ctx.tenant).findById(apiId) flatMap {
                  ...
                  case Some(api) => {
                    ...
                    otoroshiClient.createApiKey(serviceGroup, apiKey)(settings).map {
  ...
  def fakeApiCall(teamId: String, apiId: String) = DaikokuAction.async(parse.json) { ctx =>

    import scala.concurrent.duration._

    env.dataStore.teamRepo.forTenant(ctx.tenant).findByIdOrHrId(teamId).flatMap {
      case None => FastFuture.successful(NotFound(Json.obj("error" -> "Team not found")))
      case Some(team) => {
        env.dataStore.apiRepo.forTenant(ctx.tenant).findByIdOrHrId(apiId).flatMap {
          case None => FastFuture.successful(NotFound(Json.obj("error" -> "Api not found")))
          case Some(api) if !api.testing.enabled => ???
          case Some(api) if api.testing.enabled=> {
            val url = (ctx.request.body \ "url").as[String]
            val headers = (ctx.request.body \ "headers").asOpt[Map[String, String]].getOrElse(Map.empty[String, String])
            val method = (ctx.request.body \ "method").asOpt[String].getOrElse("GET")
            val body = (ctx.request.body \ "body").asOpt[String]
            val credentials = (ctx.request.body \ "credentials").asOpt[String].getOrElse("same-origin")
            val uri = Uri(url)
            val queryOpt = uri.query().toIndexedSeq.find(_._2 == s"fake-${api.id.value}")
            ...
            val finalUrl = api.testing.auth match {
              case TestingAuth.ApiKey if queryOpt.isDefined => url.replace(s"&${queryOpt.get._1}=fake-${api.id.value}", s"&${queryOpt.get._1}=${username}").replace(s"?${queryOpt.get._1}=fake-${api.id.value}", s"?${queryOpt.get._1}=${username}")
            ...
            val finalHeaders: Map[String, String] = api.testing.auth match {
              case TestingAuth.ApiKey if headerOpt.isDefined => headers - headerOpt.get._1 + (headerOpt.get._1 -> username)
              case TestingAuth.Basic => headers - "Authorization" + ("Authorization" -> s"Basic ${Base64.encodeBase64String(s"${username}:${password}".getBytes(Charsets.UTF_8))}")
              case _ => headers
            }
            ...
            body.map(b => builder.withBody(play.api.libs.ws.InMemoryBody(ByteString(b)))).getOrElse(builder)
              .stream()
              .map { res =>
