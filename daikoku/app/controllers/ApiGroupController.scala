...
class ApiGroupController(
  ...
  def getApiGroupOfTeam(teamId: String, groupId: String) = DaikokuAction.async {
    ctx =>
      TeamMemberOnly(AuditTrailEvent(
        s"@{user.name} has accessed to ApiGroup @{group.name}"))(teamId, ctx) {
        _ =>
  ...
  def getApi(api: Api, ctx: DaikokuActionContext[AnyContent]) = {
    ...
    val r: EitherT[Future, Result, Result] = for {
      ...
      error: EitherT[Future, Result, Seq[Notification]] = EitherT
        .leftT[Future, Seq[Notification]](AppError.ApiGroupNotFound.render())
        ...
        env.dataStore.notificationRepo
          ...
          .findNotDeleted(
            Json.obj(
              ...
              "action.team" -> Json.obj(
                "$in" -> JsArray(myTeams.map(_.id.asJson)))
            )
          )
      )
  ...
  def getVisibleApiWithId(apiId: String) = DaikokuActionMaybeWithGuest.async {
    ctx =>
      UberPublicUserAccess(
        AuditTrailEvent(
          "@{user.name} is accessing visible apigroup @{apigroup.name}"))(ctx) {
  ...
  def verifyNameUniqueness() = DaikokuAction.async(parse.json) { ctx =>
    PublicUserAccess(
      AuditTrailEvent(
        s"@{user.name} is checking if api group name (@{api.name}) is unique")
    )(ctx) {
  ...
  def createApiGroup(teamId: String) = DaikokuAction.async(parse.json) { ctx =>
    TeamApiEditorOnly(AuditTrailEvent(
      s"@{user.name} has created new API group @{apiGroup.id}/@{apiGroup.name}"))(
      teamId,
      ctx) { team =>
      ...
      ctx.tenant.creationSecurity match {
        ...
        case _ =>
          ApiGroupFormat.reads(finalBody) match {
            case JsError(e) =>
  ...
  def updateApiGroup(teamId: String, groupId: String) =
    DaikokuAction.async(parse.json) { ctx =>
      TeamApiEditorOnly(AuditTrailEvent(
        s"@{user.name} has updated API group @{apiGroup.name}"))(teamId, ctx) {
        team =>
          env.dataStore.apiGroupRepo
            .forTenant(ctx.tenant)
            .findByIdOrHrIdNotDeleted(groupId)
            .flatMap {
              case None => AppError.ApiGroupNotFound.renderF()
              case Some(oldGroup) if oldGroup.team != team.id =>
                AppError.ApiGroupNotFound.renderF()
              case Some(oldGroup) =>
                ApiGroupFormat.reads(ctx.request.body) match {
                  case JsError(e) =>
                    AppError.ParsingPayloadError(e.toString()).renderF()
                  case JsSuccess(group, _)
                      if !ctx.tenant.aggregationApiKeysSecurity.exists(identity) &&
                        group.possibleUsagePlans.exists(plan =>
  ...
  def deleteApiGroup(teamId: String, groupId: String) =
    DaikokuAction.async(parse.json) { ctx =>
      implicit val mat: Materializer = env.defaultMaterializer
      TeamApiEditorOnly(
        AuditTrailEvent(s"@{user.name} has deleted API group @{group.name}"))(
        teamId,
        ctx) { team =>
        env.dataStore.apiGroupRepo
          .forTenant(ctx.tenant.id)
            ...
            case Some(apiGroup) =>
              Source(apiGroup.possibleUsagePlans.toList)
                .mapAsync(1)(plan => {
                  env.dataStore.apiSubscriptionRepo
                    .forTenant(ctx.tenant)
                    .findNotDeleted(Json.obj("api" -> apiGroup.id.asJson,
                                             "plan" -> plan.id.asJson))
                    .map(subs => (plan, subs))
                })
                .via(
                  apiService.deleteApiSubscriptionsAsFlow(tenant = ctx.tenant,
                                                          apiOrGroupName =
                                                            apiGroup.name,
                                                          user = ctx.user))
                .runWith(Sink.ignore)
                .flatMap(_ =>
                  env.dataStore.apiGroupRepo
                    .forTenant(ctx.tenant.id)
                    .deleteByIdLogically(apiGroup.id))
