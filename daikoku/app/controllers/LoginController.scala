...
class LoginController(DaikokuAction: DaikokuAction,
  ...
  def loginPage(provider: String) = DaikokuTenantAction.async { ctx =>
    AuthProvider(provider) match {
      case None => Errors.craftResponseResult("Bad authentication provider", Results.BadRequest, ctx.request, None, env, ctx.tenant)
      case Some(p) if ctx.tenant.authProvider != p =>
        Errors.craftResponseResult("Bad authentication provider", Results.BadRequest, ctx.request, None, env, ctx.tenant)
      case Some(p) => p match {
        case Otoroshi => FastFuture.successful(Redirect("/"))
        case OAuth2 => {
          implicit val req = ctx.request
          val authConfig   = OAuth2Config.fromJson(ctx.tenant.authProviderSettings).toOption.get
        ...
        case _ => FastFuture.successful(Ok(views.html.login(p, ctx.tenant, ctx.request.domain, env)))
      }
    }
  }
  ...
  def bindUser(sessionMaxAge: Int,
    ...
    f.flatMap {
      case None => Errors.craftResponseResult("User not found", Results.BadRequest, request, None, env, tenant)
      case Some(user) => {
  ...
  def actualLogin(provider: String, ctx: DaikokuTenantActionContext[AnyContent]): Future[Result] = {
    AuthProvider(provider) match {
      case None => Errors.craftResponseResult("Bad authentication provider", Results.BadRequest, ctx.request, None, env, ctx.tenant)
      case Some(p) if ctx.tenant.authProvider != p =>
        Errors.craftResponseResult("Bad authentication provider", Results.BadRequest, ctx.request, None, env, ctx.tenant)
      case Some(p) => {
        ctx.request.body.asFormUrlEncoded match {
          case None => Errors.craftResponseResult("No credentials found", Results.BadRequest, ctx.request, None, env, ctx.tenant)
          case Some(form) => {
                ...
                p match {
                  ...
                  case AuthProvider.Otoroshi => {
                    ...
                    FastFuture.successful(
                      Redirect(ctx.request.session.get("redirect").getOrElse("/")).removingFromSession(
                        "redirect"
                      )(ctx.request)
                    )
                  }
                  ...
                  case AuthProvider.OAuth2           => {
                    val authConfig = OAuth2Config.fromJson(ctx.tenant.authProviderSettings).right.get
                    bindUser(authConfig.sessionMaxAge,
                      ctx.tenant,
                      ctx.request,
                      OAuth2Support.bindUser(ctx.request, authConfig, ctx.tenant, env).map(_.toOption))
                  }
                }
              }
              case _ => {
                Errors.craftResponseResult("No credentials found", Results.BadRequest, ctx.request, None, env, ctx.tenant)
              }
            }
          }
        }
  ...
  def logout(provider: String) = DaikokuAction.async { ctx =>
    val host = ctx.request.headers.get("Otoroshi-Proxied-Host").orElse(ctx.request.headers.get("X-Forwarded-Host")).getOrElse(ctx.request.host)
    val redirect =
      ctx.request.getQueryString("redirect").getOrElse(s"${ctx.request.theProtocol}://$host/")
    AuthProvider(provider) match {
      case Some(AuthProvider.Otoroshi) =>
        FastFuture.successful(Redirect(s"/.well-known/otoroshi/logout?redirect=$redirect"))
      case Some(AuthProvider.OAuth2) =>
        ...
        env.dataStore.userSessionRepo.deleteById(session.id).map { _ =>
          val cfg = OAuth2Config.fromJson(ctx.tenant.authProviderSettings).right.get
          val actualRedirectUrl = cfg.logoutUrl.replace("${redirect}", URLEncoder.encode(redirect, "UTF-8"))
          Redirect(actualRedirectUrl).removingFromSession("sessionId")(ctx.request)
        }
      case _ => {
        val session = ctx.request.attrs(IdentityAttrs.SessionKey)
        env.dataStore.userSessionRepo.deleteById(session.id).map { _ =>
  ...
  def userLogout() = DaikokuAction { ctx =>
    Redirect(fr.maif.otoroshi.daikoku.ctrls.routes.LoginController.logout(ctx.tenant.authProvider.name))
  }

  ///////// Local login module routes /////////////

  val passwordPattern = java.util.regex.Pattern.compile("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[#$^+=!*()@%&]).{8,1000}$")

  def validateUserCreationForm(name: String, email: String, password1: String, password2: String): Either[String, Unit] = {
    ...
    if (name.trim().isEmpty()) {
      ...
      (password1.trim(), password2.trim()) match {
        case (pwd1, pwd2) if pwd1 != pwd2 => Left("Your passwords does not match")
        case (pwd1, pwd2) if pwd1.isEmpty || pwd2.isEmpty => Left("Your password can't be empty")
        case (pwd1, _)    if passwordPattern.matcher(pwd1).matches() => Right(())
        case _ => Left("Your password should be longer than 8 characters and contains letters, capitalized letters, numbers and special characters (#$^+=!*()@%&) !")
      }
    }
  }
  ...
  def createUser() = DaikokuTenantAction.async(parse.json) { ctx =>
    ...
    val avatar = (body \ "avatar").asOpt[String].getOrElse("/assets/images/anonymous.jpg")
    ...
    env.dataStore.userRepo.findOne(Json.obj("email" -> email)).flatMap {
      case Some(_) => FastFuture.successful(BadRequest(Json.obj("error" -> "Email address already exists")))
      case None => {
        validateUserCreationForm(name, email, password1, password2) match {
          case Left(msg) => FastFuture.successful(BadRequest(Json.obj("error" -> msg)))
          case Right(_) => {
            val randomId = IdGenerator.token(128)
            env.dataStore.accountCreationRepo.save(AccountCreation(
              id = MongoId(BSONObjectID.generate().stringify),
              randomId = randomId,
              email = email,
              name = name,
  ...
  def createUserValidation() = DaikokuTenantAction.async { ctx =>
    ctx.request.getQueryString("id") match {
      case None => FastFuture.successful(BadRequest(Json.obj("error" -> "Id not found")))
      case Some(id) => {
        env.dataStore.accountCreationRepo.findOneNotDeleted(Json.obj("randomId" -> id)).flatMap {
          case Some(accountCreation) if accountCreation.validUntil.isBefore(DateTime.now()) => {
            env.dataStore.accountCreationRepo.deleteByIdLogically(accountCreation.id.value).map { _ =>
              Redirect("/signup?error=not-valid-anymore")
            }
          }
          case Some(accountCreation) if accountCreation.validUntil.isAfter(DateTime.now()) => {
            env.dataStore.userRepo.findOne(Json.obj("email" -> accountCreation.email)).flatMap {
              case Some(_) => FastFuture.successful(BadRequest(Json.obj("error" -> "Email address already exists")))
              case None => {
                ...
                val userCreation = for {
                  _ <- env.dataStore.teamRepo.forTenant(ctx.tenant.id).save(team)
                  _ <- env.dataStore.userRepo.save(user)
                  _ <- env.dataStore.accountCreationRepo.deleteByIdLogically(accountCreation.id.value)
                } yield ()
                userCreation.map { user =>
                  Status(302)(Json.obj("Location" -> "/")).withHeaders("Location" -> "/")
                }
              }
            }
          }
          case _ => FastFuture.successful(BadRequest(Json.obj("error" -> "Bad creation id")))
        }
      }
    }
  }
  ...
  def askForPasswordReset() = DaikokuTenantAction.async(parse.json) { ctx =>
    ...
    env.dataStore.userRepo.findOne(Json.obj("email" -> email)).flatMap {
      ...
      case None => FastFuture.successful(BadRequest(Json.obj("error" -> "Email address does not exist")))
    }
  }
  ...
  def passwordResetValidation() = DaikokuTenantAction.async { ctx =>
    ctx.request.getQueryString("id") match {
      case None => FastFuture.successful(BadRequest(Json.obj("error" -> "Id not found")))
      case Some(id) => {
        env.dataStore.passwordResetRepo.findOneNotDeleted(Json.obj("randomId" -> id)).flatMap {
          case Some(pwdReset) if pwdReset.validUntil.isBefore(DateTime.now()) => {
            env.dataStore.passwordResetRepo.deleteByIdLogically(pwdReset.id.value).map { _ =>
              Redirect("/reset?error=not-valid-anymore")
            }
          }
          case Some(pwdReset) if pwdReset.validUntil.isAfter(DateTime.now()) => {
            env.dataStore.userRepo.findOneNotDeleted(Json.obj("_id" -> pwdReset.user.value, "email" -> pwdReset.email)).flatMap {
              case None => FastFuture.successful(NotFound(Json.obj("error" -> "User not found")))
              case Some(user) => {
                env.dataStore.userRepo.save(user.copy(password = Some(pwdReset.password))).flatMap { _ =>
                  env.dataStore.passwordResetRepo.deleteByIdLogically(pwdReset.id.value).map { _ =>
                    Redirect("/")
                  }
                }
