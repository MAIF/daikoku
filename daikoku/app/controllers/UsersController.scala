...
class UsersController(
        ...
        (ctx.request.body \ "isDaikokuAdmin").asOpt[Boolean] match {
          case Some(isDaikokuAdmin) =>
            env.dataStore.userRepo.findByIdNotDeleted(id).flatMap {
              ...
              case None =>
                FastFuture
                  .successful(NotFound(Json.obj("error" -> "User not found")))
            }
          case None =>
  ...
  def deleteUserById(id: String) =
    DaikokuAction.async { ctx =>
      DaikokuAdminOnly(
        AuditTrailEvent(
          "@{user.name} has deleted user profile of user.id : @{u.id}"
        )
      )(ctx) {
  ...
  def createUser() =
    DaikokuAction.async(parse.json) { ctx =>
      DaikokuAdminOnly(
        AuditTrailEvent(
          "@{user.name} has created user profile of @{u.email} (@{u.id})"
        )
      )(ctx) {
        json.UserFormat.reads(ctx.request.body) match {
    ...
    DaikokuAction.async { ctx =>
      DaikokuAdminOnly(
        ...
        env.dataStore.userRepo.findByIdNotDeleted(userId).flatMap {
          ...
          case None =>
            FastFuture
              .successful(BadRequest(Json.obj("error" -> "User not found")))
        }
      }
    }
  ...
  def get2faQrCode() =
    DaikokuAction.async { ctx =>
      PublicUserAccess(
        AuditTrailEvent("@{user.name} clicked on enable 2fa account")
      )(ctx) {
        val label = "Daikoku"
  ...
  def enable2fa(code: Option[String]) =
    DaikokuAction.async { ctx =>
      PublicUserAccess(
        ...
        code match {
          ...
          case Some(code) =>
            ...
            if (code == totp.generateOneTimePassword(key, now).toString) {
              ...
              val backupCodes =
                new Base32()
                  .encodeAsString(keyGenerator.generateKey().getEncoded)
              env.dataStore.userRepo
                .save(
                  ctx.user.copy(
                    twoFactorAuthentication = Some(
  ...
  def disable2fa() =
    DaikokuAction.async { ctx =>
      PublicUserAccess(
        AuditTrailEvent("@{user.name} has disabled 2fa on your account")
      )(ctx) {
        env.dataStore.userRepo
  ...
  def checkTokenInvitation() =
    DaikokuActionMaybeWithGuest.async(parse.json) { ctx =>
      UberPublicUserAccess(
        AuditTrailEvent(
          "@{user.name} has tried to validate an invitation token"
        )
      )(ctx) {
  ...
  def updatePassword() =
    DaikokuAction.async(parse.json) { ctx =>
      PublicUserAccess(
        AuditTrailEvent("@{user.name} has updated his password")
      )(ctx) {
        val body = ctx.request.body
        (
