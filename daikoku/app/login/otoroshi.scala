...
object OtoroshiIdentityFilter {
  ...
  def findUserTeam(tenantId: TenantId, user: User)(implicit ec: ExecutionContext, env: Env): Future[Option[Team]] = {
    for {
      teamRepo <- env.dataStore.teamRepo.forTenantF(tenantId)
      maybePersonnalTeam <- teamRepo.findOne(Json.obj("type" -> TeamType.Personal.name, "users.userId" -> user.id.value, "_deleted" -> false))
  ...
  def apply(env: Env, tenant: Tenant, nextFilter: RequestHeader => Future[Result], request: RequestHeader)(
    ...
    val claimHeaderName     = tenant.authProviderSettings.\("claimHeaderName").as[String]
    ...
    request.headers.get(claimHeaderName) match {
      case None => {
        Errors.craftResponseResult("Not authorized here", Results.Unauthorized, request, None, env, tenant)
      }
      case Some(claim) => {
        val jwt = otoroshiJwtVerifier.verify(claim)
        Option(jwt.getClaim("email")).map(_.asString()) match {
          case None =>
            Errors.craftResponseResult("No email provided", Results.BadRequest, request, None, env, tenant)
          case Some(email) => {
            ...
            val isDaikokuAdmin: Boolean =
              Option(jwt.getClaim("daikokuAdmin"))
                .flatMap(a => Option(a.asString()))
                .exists(_.toBoolean)
            def createSessionFromOtoroshi(maybeUser: Option[User] = None, maybeSession: Option[UserSession] = None): Future[Result] = {
              maybeUser match {
                case None => {
                  env.dataStore.userRepo.findOne(Json.obj("_deleted" -> false, "email" -> email)).flatMap {
                    case None => {
                      ...
                      val session = maybeSession.getOrElse(UserSession(
                      ...
                      for {
                        _ <- env.dataStore.teamRepo.forTenant(tenant.id).save(newTeam)
                        _ <- env.dataStore.userRepo.save(defaultUser)
                        impersonator <- session.impersonatorId.map(id => env.dataStore.userRepo.findByIdNotDeleted(id)).getOrElse(FastFuture.successful(None))
                        rr <- nextFilter(
                          request
                            .addAttr(IdentityAttrs.TeamKey, newTeam)
                            .addAttr(IdentityAttrs.UserKey, defaultUser)
                        ...
                        r <- FastFuture.successful(rr.removingFromSession("sessionId")(request).withSession("sessionId" -> session.sessionId.value))
                      } yield {
                        r
                      }
                    }
                    case Some(u) => {
                      ...
                      for {
                        ...
                        userTeam <- if (userTeamOpt.isDefined) FastFuture.successful(userTeamOpt.get) else env.dataStore.teamRepo.forTenant(tenant.id).save(updatedTeam).map(_ => updatedTeam)
            ...
            request.session.get("sessionId").orElse(request.getQueryString("sessionId")) match {
              ...
              case Some(sessionId) => env.dataStore.userSessionRepo.findOne(Json.obj("sessionId" -> sessionId)).flatMap {
                case None => createSessionFromOtoroshi()
                case Some(session) if session.expires.isBefore(DateTime.now()) => createSessionFromOtoroshi()
                case Some(session) if session.expires.isAfter(DateTime.now()) => {
                  env.dataStore.userRepo.findByIdNotDeleted(session.userId).flatMap {
                    case None => createSessionFromOtoroshi(maybeSession = Some(session))
                    case Some(user) => createSessionFromOtoroshi(maybeUser = Some(user), maybeSession = Some(session))
                  }
                }
              }
            }
