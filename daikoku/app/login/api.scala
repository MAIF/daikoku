...
object IdentityAttrs {
  ...
  val ImpersonatorKey: TypedKey[Option[User]] = TypedKey.apply[Option[User]]("daikoku-impersonator")
  val TenantKey: TypedKey[Tenant]             = TypedKey.apply[Tenant]("daikoku-tenant")
  val SessionKey: TypedKey[UserSession]       = TypedKey.apply[UserSession]("daikoku-session")
...
object TenantHelper {
  ...
  def extractTenantId(request: RequestHeader)(implicit env: Env): TenantId = {
    request.headers
      .get("Daikoku-Tenant")
      .flatMap(t => Option(env.config.tenantJwtVerifier.verify(t).getClaim("value").asString()))
      .map(TenantId.apply)
      .getOrElse(Tenant.Default)
  }

  def withTenant(request: RequestHeader, env: Env)(f: Tenant => Future[Result])(implicit ec: ExecutionContext): Future[Result] = {
    ...
    env.config.tenantProvider match {
      case TenantProvider.Header => {
        ...
        env.dataStore.tenantRepo.findByIdNotDeleted(tenantId).flatMap {
          case None => Errors.craftResponseResult("Tenant does not exists", Results.NotFound, request, None, env)
          case Some(tenant) if !tenant.enabled =>
            Errors.craftResponseResult("Tenant does not exists", Results.NotFound, request, None, env, tenant)
          case Some(tenant) => f(tenant)
        }
      }
      case TenantProvider.Hostname => {
        val host = request.headers.get("Otoroshi-Proxied-Host").orElse(request.headers.get("X-Forwarded-Host")).getOrElse(request.host)
        val domain = if (host.contains(":")) host.split(":").apply(0) else host
        env.dataStore.tenantRepo.findOne(Json.obj(
          "_deleted" -> false,
          "domain" -> domain
        )).flatMap {
          case None => Errors.craftResponseResult(s"Tenant does not exists", Results.NotFound, request, None, env)
          case Some(tenant) if !tenant.enabled =>
            Errors.craftResponseResult("Tenant does not exists", Results.NotFound, request, None, env, tenant)
          case Some(tenant) => f(tenant)
        }
      ...
      case TenantProvider.Local => {
        ...
        val tenantIdF: Future[TenantId] = request.session.get("sessionId") match {
          case None =>
            FastFuture.successful(Tenant.Default)
          case Some(sessionId) => {
            env.dataStore.userSessionRepo.findOne(Json.obj("sessionId" -> sessionId)).flatMap {
              case None =>
                FastFuture.successful(Tenant.Default)
              case Some(session) if session.expires.isBefore(DateTime.now()) =>
                FastFuture.successful(Tenant.Default)
              case Some(session) if session.expires.isAfter(DateTime.now()) => {
                env.dataStore.userRepo.findByIdNotDeleted(session.userId).flatMap {
                  case None =>
                    FastFuture.successful(Tenant.Default)
                  case Some(user) => {
        ...
        tenantIdF.flatMap(env.dataStore.tenantRepo.findByIdNotDeleted(_)).flatMap {
          case None => Errors.craftResponseResult("Tenant does not exists", Results.NotFound, request, None, env)
          case Some(tenant) if !tenant.enabled => Errors.craftResponseResult("Tenant does not exists", Results.NotFound, request, None, env, tenant)
          case Some(tenant) => f(tenant)
        }
      }
    }
...
class LoginFilter(env: Env)(implicit val mat: Materializer, ec: ExecutionContext) extends Filter {
  ...
  def findUserTeam(tenantId: TenantId, user: User): Future[Option[Team]] = {
    ...
    for {
      ...
      theMaybeTeam: Option[Team] <- if (maybePersonnalTeam.isDefined) FastFuture.successful(maybePersonnalTeam) else teamRepo.save(backupTeam).map(_ => Some(backupTeam))
  ...
  def apply(nextFilter: RequestHeader => Future[Result])(request: RequestHeader): Future[Result] = {
    ...
    logger.debug(s"Filtering on ${request.method.toLowerCase()} => ${request.relativeUri}")
        ...
        TenantHelper.withTenant(request, env) { tenant =>
          ...
          def passWithSession(): Future[Result] = {
            request.session.get("sessionId").orElse(request.getQueryString("sessionId")) match {
              case None =>
                (request.method.toLowerCase(), request.relativeUri) match {
                  case ("get", r"/") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case ("get", r"/signup") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case ("get", r"/reset") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case ("get", path) if path.startsWith("/reset") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case ("get", path) if path.startsWith("/signup") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case (_, r"/account") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case (_, r"/account/$path@(.*)") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case ("get", r"/tenant-assets/$path@(.*)") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case ("get", r"/user-assets/$path@(.*)") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case ("get", r"/asset-thumbnails/$path@(.*)") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case (_, r"/admin-api/$path@(.*)") => nextFilter(request.addAttr(IdentityAttrs.TenantKey, tenant))
                  case (_, r"/integration-api/$path@(.*)") => {
                    request.getQueryString("token").orElse(request.headers.get("X-Personal-Token")) match {
                      case None => {
                      ...
                      case Some(token) => {
                        env.dataStore.userRepo.findOneNotDeleted(Json.obj("personalToken" -> token)).flatMap {
                          case None => {
                          ...
                          case Some(_user) => {
                            ...
                            findUserTeam(tenant.id, user).flatMap {
                              case None => logger.info("No team found")
                                FastFuture.successful(
                                  Results
                                    .Redirect(fr.maif.otoroshi.daikoku.ctrls.routes.LoginController.loginPage(provider.name))
                                ...
                                env.dataStore.userRepo.save(user).flatMap { _ =>
                                  nextFilter(
                                    request
                                      ...
                                      .addAttr(IdentityAttrs.ImpersonatorKey, None)
                      ...
                      case Some(_user) => {
                        ...
                        findUserTeam(tenant.id, user).flatMap {
                          ...
                          case Some(team) => for {
                            ...
                            result  <- session.impersonatorId.map(id => env.dataStore.userRepo.findByIdNotDeleted(id))
                                ...
                                .flatMap { maybeImpersonator =>
                                  nextFilter(
                                    request
                                        .addAttr(IdentityAttrs.TeamKey, team)
                                        .addAttr(IdentityAttrs.UserKey, user)
                                        .addAttr(IdentityAttrs.ImpersonatorKey, maybeImpersonator)
                                        .addAttr(IdentityAttrs.TenantKey, tenant)
                                        .addAttr(IdentityAttrs.SessionKey, session)
                                  )
                                }
          ...
          provider match {
            ...
            case AuthProvider.Otoroshi         => OtoroshiIdentityFilter(env, tenant, nextFilter, request)
