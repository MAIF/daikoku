...
object OAuth2Support {
  ...
  def bindUser(request: RequestHeader, authConfig: OAuth2Config, tenant: Tenant, _env: Env)(
          ...
          case Some(code) => {
            ...
            future1.flatMap { resp =>
              ...
              if (authConfig.readProfileFromToken && authConfig.jwtVerifier.isDefined) {
                ...
                algoSettings.asAlgorithmF(InputMode(alg, kid))(_env, ec).flatMap {
                  case Some(algo) => {
                ...
                val name = (userFromOauth \ authConfig.nameField).asOpt[String].getOrElse("No Name")
                val email = (userFromOauth \ authConfig.emailField).asOpt[String].getOrElse("no.name@foo.bar")
                _env.dataStore.userRepo.findOne(Json.obj("_deleted" -> false, "email" -> email)).flatMap {
                  case None => {
