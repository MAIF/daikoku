...
import fr.maif.audit.AuditActorSupervizer
import fr.maif.domain.TeamPermission.Administrator
import fr.maif.domain.Tenant.getCustomizationCmsPage
import fr.maif.domain.{DatastoreId, ReportsInfo, TeamApiKeyVisibility, Tenant}
import fr.maif.logger.AppLogger
import fr.maif.login.AuthProvider.Local
import fr.maif.login.{AuthProvider, LoginFilter, OAuth2Config}
          ...
          val eventualUnit: Future[Unit] =
            path.orElse(config.init.data.from) match {
              ...
              case _ =>
                ...
                import fr.maif.utils.StringImplicits.*
