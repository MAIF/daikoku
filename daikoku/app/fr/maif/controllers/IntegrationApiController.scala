...
import fr.maif.actions._
import fr.maif.audit.AuditTrailEvent
import fr.maif.controllers.authorizations.async.TenantAdminAccessTenant
import fr.maif.domain._
import fr.maif.env.Env
import fr.maif.utils.StringImplicits._
import fr.maif.utils.future._
...
class IntegrationApiController(
  ...
  def api(teamId: String, apiId: String) =
    DaikokuAction.async { ctx =>
      TenantAdminAccessTenant(
        AuditTrailEvent(
  ...
  def apiComplete(teamId: String, apiId: String) =
    DaikokuAction.async { ctx =>
      TenantAdminAccessTenant(
        AuditTrailEvent(
        ...
        (for {
          ...
          api._2.documentation.fetchPages(ctx.tenant).map { pages =>
            ...
            val newDoc = api._2.documentation.asJson
              .as[
                JsObject
              ] - "_id" - "_humanReadableId" - "lastModificationAt" ++ Json
              .obj("pages" -> JsArray(cleanPages))
            Ok(
              api._2.asJson
                .as[
                  JsObject
                ] - "tenant" - "_deleted" - "_humanReadableId" - "subscriptions" - "authorizedTeams" - "managedServices" ++ Json
                .obj(
  ...
  def apiPlans(teamId: String, apiId: String) =
    DaikokuAction.async { ctx =>
      TenantAdminAccessTenant(
        AuditTrailEvent(
  ...
  def apiSwagger(teamId: String, apiId: String) =
    DaikokuAction.async { ctx =>
      TenantAdminAccessTenant(
        AuditTrailEvent(
