...
import fr.maif.daikoku.domain.{CmsPageId, Tenant}
...
class Translator {
  ...
  def renderTranslationAsCmsPage(
      value: Future[String],
      tenant: Tenant,
      args: Map[String, JsValue] = Map.empty
  )(implicit
  ...
  def _getMailTemplate(key: String, tenant: Tenant, args: Map[String, JsValue])(
      implicit
      language: String,
      env: Env,
      messagesApi: MessagesApi
  ...
  def getMailTemplate(key: String, tenant: Tenant, args: Map[String, JsValue])(
      implicit
      language: String,
      env: Env,
      messagesApi: MessagesApi
