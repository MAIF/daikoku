...
object playJson extends PlayJsonSupportLowPrioImplicits {
  implicit object PlayJsonResultMarshaller extends ResultMarshaller {
    ...
    override def enumNode(value: String, typeName: String): Node =
      JsString(value)
  ...
  implicit object PlayJsonMarshallerForType
      extends ResultMarshallerForType[JsValue] {
  ...
  implicit object PlayJsonInputUnmarshaller extends InputUnmarshaller[JsValue] {
    ...
    override def getListValue(node: JsValue): Seq[JsValue] =
      node.asInstanceOf[JsArray].value.toSeq
    ...
    override def getMapValue(node: JsValue, key: String): Option[JsValue] =
      node.asInstanceOf[JsObject].value.get(key)
    override def getMapKeys(node: JsValue): Iterable[String] =
      node.asInstanceOf[JsObject].keys

    override def isDefined(node: JsValue): Boolean = node != JsNull
    override def getScalarValue(node: JsValue): Any = {
      node match {
        case JsBoolean(b) => b
