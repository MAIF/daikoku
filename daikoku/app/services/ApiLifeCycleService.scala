...
class ApiLifeCycleService(mailService: MailService, otoroshiClient: OtoroshiClient) {
    ...
    for {
      _ <- EitherT.liftF(
             modifyApiState(
               api = api,
               apiEnabling = true,
               subscriptionDemandState = SubscriptionDemandState.Accepted
             )
           )
      ...
      _ <- EitherT.liftF(updateApiKey(subOrNotToSub = subOrNotToSub))
    ...
    for {
      _             <- EitherT.liftF(
                         modifyApiState(
                           api = api,
                           apiEnabling = false,
                           subscriptionDemandState = SubscriptionDemandState.Blocked
                         )
                       )
      subscriptions <- EitherT.liftF(getSubscriptionNotDeleted(api = api))
      plansIds       = subscriptions.map(_.plan).distinct
  ...
  private def modifyApiState(api: Api, apiEnabling: Boolean, subscriptionDemandState: SubscriptionDemandState)(implicit
      ec: ExecutionContext,
      env: Env,
      ctx: DaikokuActionContext[JsValue]
  ) = {
    env.dataStore.apiSubscriptionRepo
      ...
      .updateManyByQuery(
        ...
        Json.obj(
          "$set" -> Json
            .obj("state" -> subscriptionDemandState.name, "enabled" -> apiEnabling)
        )
      )
  }
  ...
  private def updateApiKey(subOrNotToSub: (Seq[ApiSubscription], Seq[(ApiSubscription, OtoroshiSettings)]))(implicit
      ec: ExecutionContext
  ) = {
    Future.sequence(
      subOrNotToSub._2.map { case (sub, os) =>
        val future: Future[Either[AppError, ActualOtoroshiApiKey]] =
          otoroshiClient.getApikey(sub.apiKey.clientId)(os).flatMap {
            ...
            case Right(apikey) => otoroshiClient.updateApiKey(apikey.copy(enabled = true))(os) //FIXME: use patch
          }
        future
      }
    )
