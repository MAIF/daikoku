...
case class DataConfig(from: Option[String], headers: Map[String, String] = Map.empty[String, String])
...
object InitConfig {
  def apply(configuration: Configuration): InitConfig = {
    ...
    InitConfig(
      host =  configuration.getOptional[String]("daikoku.init.host").getOrElse("localhost"),
      AdminConfig(
        name = configuration.getOptional[String]("daikoku.init.admin.name").getOrElse("Super admin"),
        email = configuration.getOptional[String]("daikoku.init.admin.email").getOrElse("admin@otoroshi.io"),
        password = configuration.getOptional[String]("daikoku.init.admin.password").getOrElse(generatedPassword)
      ),
      DataConfig(
        from = configuration.getOptional[String]("daikoku.init.data.from"),
        headers = configuration.getOptional[Map[String, String]]("daikoku.init.data.headers").getOrElse(Map.empty[String, String])
      )
    )
  }
}
...
case class OtoroshiAdminApiConfig(claimsHeaderName: String, algo: Algorithm) extends AdminApiConfig

object AdminApiConfig {
  def apply(config: Configuration): AdminApiConfig = {
    config.getOptional[String]("daikoku.api.type") match {
      case Some("local") => LocalAdminApiConfig(config.getOptional[String]("daikoku.api.key").get)
      case Some("otoroshi") => OtoroshiAdminApiConfig(
        config.getOptional[String]("daikoku.api.headerName").get,
        Algorithm.HMAC512(config.getOptional[String]("daikoku.api.headerSecret").get)
      )
      case _ => LocalAdminApiConfig(config.getOptional[String]("daikoku.api.key").get)
    }
  }
}
...
class Config(val underlying: Configuration) {
  ...
  lazy val tenantProvider = underlying.getOptional[String]("daikoku.tenants.provider").flatMap(TenantProvider.apply).getOrElse(TenantProvider.Local)

  lazy val otoroshiSyncInterval: FiniteDuration = underlying.getOptional[Long]("daikoku.otoroshi.sync.interval").map(v => v.millis).getOrElse(1.hour)
  lazy val otoroshiSyncMaster = underlying.getOptional[Boolean]("daikoku.otoroshi.sync.master").getOrElse(
    underlying.getOptional[Int]("daikoku.otoroshi.sync.instance").getOrElse(-1) == 0
  )
  lazy val otoroshiSyncByCron = underlying.getOptional[Boolean]("daikoku.otoroshi.sync.cron").getOrElse(false)
  lazy val otoroshiSyncKey = underlying.getOptional[String]("daikoku.otoroshi.sync.key").getOrElse("secret")
  lazy val otoroshiGroupNamePrefix: Option[String] = underlying.getOptional[String]("daikoku.otoroshi.groups.namePrefix")
  lazy val otoroshiGroupIdPrefix: Option[String] = underlying.getOptional[String]("daikoku.otoroshi.groups.idPrefix")

  lazy val apikeysStatsByCron = underlying.getOptional[Boolean]("daikoku.stats.sync.cron").getOrElse(false)
  lazy val apikeysStatsSyncInterval: FiniteDuration = underlying.getOptional[Long]("daikoku.stats.sync.interval").map(v => v.millis).getOrElse(1.hour)
  lazy val apikeysStatsCallInterval: FiniteDuration = underlying.getOptional[Long]("daikoku.stats.call.interval").map(v => v.millis).getOrElse(10.minutes)
...
sealed trait Env {
  ...
  def config: Config
  def identityFilters(implicit mat: Materializer, ec: ExecutionContext): Seq[EssentialFilter]
  def expositionFilters(implicit mat: Materializer, ec: ExecutionContext): Seq[EssentialFilter]
}

class DaikokuEnv(ws: WSClient, val environment: Environment, configuration: Configuration, reactiveMongoApi: ReactiveMongoApi) extends Env {

  val logger = Logger("DaikokuEnv")

  val actorSystem  = ActorSystem("daikoku")
  val materializer = ActorMaterializer.create(actorSystem)
  val snowflakeSeed      = configuration.getOptional[Long]("daikoku.snowflake.seed").get
  ...
  private val s3assetsStore = new AssetsDataStore(actorSystem)(actorSystem.dispatcher, materializer)

  override def defaultExecutionContext: ExecutionContext = actorSystem.dispatcher
  ...
  override def onStartup(): Unit = {
    ...
    def tryToInitDatastore(): Future[Unit] = {
      dataStore.isEmpty().map {
        case true => config.init.data.from match {
          case Some(path) if path.startsWith("http://") ||  path.startsWith("https://") => {
            logger.warn("")
            logger.warn(s"Main dataStore seems to be empty, importing from ${path} ...")
            ...
            val initialDataFu = wsClient
              ...
              .get()
              .flatMap {
                case resp if resp.status == 200 => dataStore.importFromStream(resp.bodyAsSource)
                case resp => FastFuture.failed(new RuntimeException(s"Bad response from ${path}: ${resp.status} - ${resp.body}"))
              }
          ...
          case _ => {
            ...
            logger.warn("")
            logger.warn("Main dataStore seems to be empty, generating initial data ...")
            ...
            val user = User(
              ...
              password = Some(BCrypt.hashpw(config.init.admin.password, BCrypt.gensalt())),
            ...
            Await.result(initialDataFu, 10 seconds)
            logger.warn("")
            logger.warn(s"You can log in with admin@daikoku.io / ${config.init.admin.password}")
            logger.warn("")
            logger.warn("Please avoid using the default tenant for anything else than configuring Daikoku")
            logger.warn("")
          }
        }
        case false =>
      ...
      case Some("otoroshi") =>
        Seq(
          new OtoroshiExpositionFilter(
            configuration.get[String]("daikoku.exposition.otoroshi.stateHeaderName"),
            configuration.get[String]("daikoku.exposition.otoroshi.stateRespHeaderName"),
            this
          )(mat, ec)
        )
      case _ => Seq.empty
