...
export const TeamIncome = (props: TeamBackOfficeProps) => {
  ...
  useEffect(() => {
    if (props.currentTeam && !isError(props.currentTeam)) {
      getBillingData(date);
      document.title = `${props.currentTeam.name} - ${translate('Income')}`;
    }
  }, [props.currentTeam]);
  ...
  const getBillingData = (date: dayjs.Dayjs) => {
    ...
    Promise.all([
      ...
      client.query({
        ...
        variables: { teamId: (props.currentTeam as ITeamSimple)._id },
      }),
    ]).then(
      ([
        consumptions,
  ...
  const sync = () => {
    setState({ ...state, loading: true });
    Services.syncTeamIncome(props.currentTeam._id).then(() => getBillingData(date));
  };
  ...
  return (<Can I={read} a={api} team={props.currentTeam} dispatchError={true}>
    <div className="row">
      <div className="col">
        <h1>
