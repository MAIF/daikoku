...
export const TeamApiTranslation = ({ value, onChange }) => {
  ...
  const schema = lng => {
    ...
    const schemaObj = {
      smallDescription: { type: 'string', props: { label: 'small desc.', value: getTranslatedValue('smallDescription', lng, ""), placeholder: getTranslatedValue('smallDescription', lng)} },
      description: { type: 'code', props: { label: 'description', value: getTranslatedValue('description', lng, ""), placeholder: getTranslatedValue('description', lng) } },
    }

    //todo: value & placeholder doesn't work because of pp is an array
    value.possibleUsagePlans.forEach(pp => {
      schemaObj[`possibleUsagePlans//${pp._id}//customName`] = { type: 'string', props: { label: 'custom name', value: getTranslatedValue(`possibleUsagePlans.${pp._id}.customName`, lng, ""), placeholder: getTranslatedValue(`possibleUsagePlans.${pp._id}.customName`, lng) } }
      schemaObj[`possibleUsagePlans//${pp._id}//customDescription`] = { type: 'string', props: { label: 'custom desc.', value: getTranslatedValue(`possibleUsagePlans.${pp._id}.customDescription`, lng, ""), placeholder: getTranslatedValue(`possibleUsagePlans.${pp._id}.customDescription`, lng) } }
    })
  ...
  const handleChange = translations => {
    const t = translations.map(localizedTranslation => {
      const language = localizedTranslation.key
      // delete localizedTranslation.key
      return Object.keys(localizedTranslation)
        .filter(key => key !== 'key')
        .map(key => ({ language, key: `${value._id}.${key.replace(/\/\//g, ".")}`, value: localizedTranslation[key] }))
        .filter(t => !!t.value)
    }).flat()
    onChange(t)
  }
...
export const TeamApiTranslationForm = ({ team, api, t, onTranslationChange}) => {
  ...
  const planCustomDescriptionKey = (plan) => `${api._id}.possibleUsagesPlans.${plan._id}.customDescription`
