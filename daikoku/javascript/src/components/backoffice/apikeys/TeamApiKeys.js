...
export class TeamApiKeysComponent extends Component {
  ...
  cleanSubs = () => {
    window.confirm(t('clean.archived.sub.confirm', this.props.currentLanguage, 'Are you sure you want to clean archived subscriptions ?')).then(ok => {
      if (ok) {
        Services.cleanArchivedSubscriptions(this.props.currentTeam._id)
        ...
        <Can I={read} a={apikey} team={this.props.currentTeam} dispatchError={true}>
          <div className="row">
            <div className="col">
              ...
              />
              <button className="btn btn-sm btn-danger-negative mt-1" 
                onClick={this.cleanSubs}>
...
export const TeamApiKeys = connect(
  mapStateToProps
)(TeamApiKeysComponent);
