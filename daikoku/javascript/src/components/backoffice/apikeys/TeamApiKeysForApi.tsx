...
export interface IKeyringSubscriptionGql {
  ...
  tags: Array<string>;
  lastUsage?: number;
  plan: { _id: string; customName: string; autoRotation?: boolean; costPerMonth?: number };
...
export const ApiKeysListForApi = (props: ApiKeysListForApiProps) => {
  ...
  /** Ends the subscription when the paid period runs out. The key keeps working
    * until that last day, and the closing invoice still carries the usage. */
  const cancelSubscription = (subscription: IKeyringSubscriptionGql) =>
    confirm({
      ...
      if (ok) {
        return Services.cancelApiSubscription(
          props.team._id,
          subscription._id,
          true
        ).then(() => {
...
type KeyringCardProps = {
  ...
  cancelSubscription: (subscription: IKeyringSubscriptionGql) => void;
                  ...
                  <Can I={manage} a={apikey} team={currentTeam}>
                    <div className="dropdown">
                      ...
                      >
                        ...
                        <div className="dropdown-divider" />
                        {!!sub.plan?.costPerMonth && (
                          <button
                            className="dropdown-item cursor-pointer danger"
                            onClick={() => withLoader(() => cancelSubscription(sub))}
                          >
