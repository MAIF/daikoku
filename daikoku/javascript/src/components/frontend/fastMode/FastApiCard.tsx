...
export const ExpertApiCard = (props: ExpertApiCardProps) => {
  ...
  console.debug({props, selectedApiV})
  return (
      ...
      <div className="col-12"><span className="d-flex flex-row mx-3 justify-content-between">
        ...
        {props.apiWithAuthorization.length > 1 &&
            <Select
                ...
                onChange={(e) => {
                  console.debug({apis: props.apiWithAuthorization, value: e!.value, test: props.apiWithAuthorization.find((api) => api.api.currentVersion == e!.value)})
                  ...
                  <div className="mx-3 d-flex justify-content-between my-1">
                    ...
                    {subPlan.isPending &&
                        <button style={{whiteSpace: "nowrap"}} disabled={true}
                                className={"btn btn-outline-primary disabled me-1"}> {translate('fastMode.button.pending')} </button>}
