...
export const FastApiCard = (props: FastApiCardProps) => {
  ...
  const [selectedApiV, setSelectedApiV] = useState(
    props.apisWithAuthorization.find((a) => a.api.isDefault)?.api
      .currentVersion || props.apisWithAuthorization[0].api.currentVersion
  );
  const [selectedApi, setSelectedApi] = useState<IFastApi>(
    props.apisWithAuthorization.find(
      (api) => api.api.currentVersion === selectedApiV
    )!
  );
  ...
  const changeApiV = (version: string) => {
    ...
    setSelectedApi(
      props.apisWithAuthorization.find(
        (api) => api.api.currentVersion === version
      )!
    );
  };
      ...
      <div className="col-12">
          ...
          {props.apisWithAuthorization.length > 1 && (
            <Select
              ...
              options={props.apisWithAuthorization.map((api) => {
                return {
                  value: api.api.currentVersion,
                  label: api.api.currentVersion,
                };
              })}
              onChange={(e) => {
                changeApiV(e!.value);
        ...
        <div className="d-flex flex-column fast_api" id="usage-plans__list">
          {selectedApi.subscriptionsWithPlan
            .map((subPlan) => {
              const plan = selectedApi.api.possibleUsagePlans.find(
                (pPlan) => pPlan._id === subPlan.planId
              )!;
              return { plan, ...subPlan };
            })
            .sort((a, b) =>
              (a.plan.customName || '').localeCompare(b.plan.customName || '')
            )
            .filter(
              ({ plan }) =>
                plan.otoroshiTarget &&
            ...
            .map(({ plan, subscriptionsCount, isPending }) => {
              if (
                !plan.customName
                  ?.toLowerCase()
                  .includes(props.planResearch.toLowerCase()) ||
                plan.otoroshiTarget?.authorizedEntities === null
              ) {
                return;
              }
              return (
                <div
                  className="fast__hover plan cursor-pointer"
                  key={plan._id}
                ...
                >
                  <div className="mx-3 d-flex justify-content-between my-1">
                    ...
                    {!!subscriptionsCount && (
                      ...
                      >
                        {translate({
                          key: 'fastMode.button.seeApiKey',
                          plural: subscriptionsCount > 1,
                        })}
                      </button>
                    )}
