...
export const AdminMessages = () => {
  ...
  useEffect(() => {
    setPossibleNewUsers(
      users.filter(
        (u) =>
          !u.isDaikokuAdmin &&
          !groupedMessages.some(({ chat }) => chat === u._id)
      )
    );
  ...
  const maybeReadMessage = () => {
    if (selectedChat) {
      const unreadCount = Option(
        groupedMessages.find((g) => g.chat === selectedChat)
      )
  ...
  const dialog = Option(
    groupedMessages.find(({ chat }) => chat === selectedChat)
  )
  ...
  return (
    <div className="d-flex flex-row messages-container">
      ...
      <div className="d-flex flex-column-reverse ms-2 messages-content">
        ...
        {selectedChat &&
          lastClosedDates.find((x) => x.chat === selectedChat).date && (
