...
export const ContentSideView = ({ value, onChange, pages, publish, contentType }) => {
  ...
  useEffect(() => {
    if (ref) ref.on('mousedown', onMouseDown);
  }, [ref]);
  ...
  const filterHelpers = (value) => {
    ...
    setHelpers(
      Object.fromEntries(
        Object.entries(helpersList).map(([g, { helpers, ...rest }]) => [
          g,
          {
            ...rest,
          ...
          >
            <div className="d-flex" style={{ height: '100%', position: 'relative' }}>
                    ...
                    {Object.entries(helpersList).map(([groupName, { helpers, collapsed }]) => (
                      <div
                        onClick={() =>
                          setHelpers(
                            Object.fromEntries(
                              Object.entries(helpersList).map(([g, { collapsed, ...rest }]) => {
                                if (g === groupName)
                                  return [
                                    g,
                                    {
                                      ...rest,
                                      collapsed: !collapsed,
                                      helpers: rest.helpers.map((helper) => ({
                                        ...helper,
                                        filtered: false,
                                      })),
                                    },
                            ...
                            .map((helper) => (
                              <button
                                ...
                                style={{
                                  ...
                                  border: 'none',
                                  backgroundColor:
                                    selector?.name === helper.name ? '#bdc3c7' : '#ddd',
              ...
              <div style={{ flex: selector ? 1 : 0 }} className="ms-2 p-3">
                ...
                />
                {selector?.name === 'links' && (
                  <LinksView editor={ref} onChange={() => setSideView(false)} />
                )}
                {selector?.name === 'pages' && (
                  <PagesView
                    pages={pages}
                    prefix="daikoku-page-url"
                    title={translateMethod('cms.content_side_view.link_to_insert')}
                    editor={ref}
                    onChange={() => setSideView(false)}
                  />
                )}
                {selector?.name === 'blocks' && (
                  <PagesView
                    pages={pages}
                    prefix="daikoku-include-block"
                    title={translateMethod('cms.content_side_view.block_to_render')}
                    editor={ref}
                    onChange={() => setSideView(false)}
                  />
                )}
                {(selector?.name.startsWith('daikoku') ||
                  !['links', 'blocks', 'pages'].includes(selector?.name)) &&
                  selector && (
                    <HelperView
                      editor={ref}
                      onChange={() => setSideView(false)}
