...
export function EditFrontOfficeTranslations(props) {
  ...
  const loadTranslations = () => {
    return Services.getTranslations('all').then((store) => {
      ...
      return Object.entries(t).map(([message, translations]) => ({ message, translations }));
    });
  };
  ...
  const columns = [
    {
      ...
      Cell: ({
        ...
        return <div>{translateMethod(original.message)}</div>;
      },
    },
    {
      id: 'actions',
                  ...
                  onClick={() =>
                    dispatch(
                      openFormModal({
                        ...
                        onSubmit: (t) =>
                          updateTranslation(t).then(() => {
                            toastr.success(
                              translateMethod('mailing_internalization.translation_updated')
                            );
